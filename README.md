...
В сообществе VK включите входящие сообщения и Long Poll API. После `docker compose up` бот обрабатывает команды `/start`, `/help`, `/link` (или `/привязать`, код в том же сообщении или следующим), `/profile`, для участников с ролью `member` — сценарии `/issue` (заявка на выдачу) и `/return` (возврат), callback-кнопки «Принять / Отклонить» для завснаров. Модули: каталог `bot/bot/` (`main.py`, `flows/`, …).
